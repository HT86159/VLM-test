...
class SelfCheckBERTScore:
    ...
    def predict(
        ...
        for s in range(num_samples):
            ...
            # sample_sentences = sampled_passages[s]
            # nlp = spacy.load('en_core_web_sm')
            # sample_sentences = [sent.text.strip() for sent in nlp(sample_sentences).sents] # spacy sentence tokenization
            # cand=["I have an apple.","I am Lucky."]#一段话中两个句子
            # ref=["I have a pen.","I am Lucy."]#一段话中两个句子
            # 得到的是tensor([0.8176, 0.6489]) tensor([0.8176, 0.6489]) tensor([0.8176, 0.6489])
            # cands = sentences
            # refs = sample_sentences
