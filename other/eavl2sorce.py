...
def parse_args():
    ...
    parser.add_argument("--input", type=str, default="/data/huangtao/projects/llm-safty/MyVLMTest/qa_results/LRV/LRV-red_team1k-no_image.jsonl", help="text to input")
...
output_file = "/data/huangtao/projects/llm-safty/MyVLMTest/results/Blip2/blip2-train-with_image-202391120_perspective_detoxify.jsonl"


save_path1 = os.path.join(os.path.dirname(output_file),output_file.split("/")[-1].split(".")[0]+'perspective_api.txt')
assert not os.path.exists(save_path1),"File alreay exists"
save_path2 = os.path.join(os.path.dirname(output_file),output_file.split("/")[-1].split(".")[0]+'detoxify.txt')
